...
/**
 * 支付领域服务：根据支付结果的终态判定，驱动支付结果同步端口。
 *
 * <p>订单事件映射与幂等判定属于订单域知识，由端口的订单域适配器负责。</p>
 */
@Slf4j
@Service
...
public class PaymentDomainService {
    ...
    private final PaymentResultSyncPort paymentResultSyncPort;

    /**
     * 处理一笔支付结果：支付成功 / 交易关闭走同步端口，非终态仅记录。
     */
    public void process(PaymentResult result) {
        if (result.isSuccess()) {
            paymentResultSyncPort.syncPaid(result.getOrderNo(), result.getTradeNo(),
                    result.isSettled(), result.getRawStatus(), result.getTraceId());
        } else if (result.isClosed()) {
            paymentResultSyncPort.syncClosed(result.getOrderNo(), result.getTraceId());
        } else {
